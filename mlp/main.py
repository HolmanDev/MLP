...
class Program():
    ...
    def train(self, input_data_list, n_datapoints):
        ...
        gradient_vec = np.empty(n) # Whole gradient vector
        ...
        for data_index in range(n_datapoints):
            ...
            for j in range(self.n_out):
                for k in range(self.n_L2):
                    ...
                    gradient_vec[index+1] += self.lambda_1_bias(3, j, k) * dc_da
                    index += 2
            # Layer 2
            for j in range(self.n_L2):
                for k in range(self.n_L1):
                    ...
                    gradient_vec[index+1] += self.lambda_1_bias(2, j, k) * dc_da
                    index += 2
            # Layer 1
            for j in range(self.n_L1):
                for k in range(self.n_in):
                    ...
                    gradient_vec[index+1] += self.lambda_1_bias(1, j, k) * dc_da
                    index += 2
        gradient_vec = gradient_vec.reshape(n, 1) / n_datapoints # Now we have the final averaged gradient vector!
        ...
        self.weights_out -= gradient_vec[:2:n_total_out]
        self.bias_out -= gradient_vec[1:2:n_total_out]
        # Layer 2
        n_weights_L2 = self.n_L2 * self.n_L1
        n_bias_L2 = self.n_L2
        n_total_L2 = n_weights_L2 + n_bias_L2
        self.weights_L2 -= gradient_vec[n_total_out : 2 : n_total_out + n_total_L2]
        self.bias_L2 -= gradient_vec[n_total_out + 1 : 2 : n_total_out + n_total_L2]
        # Layer 1
        self.weights_L2 -= gradient_vec[n_total_out + n_total_L2: 2 :]
        self.bias_L2 -= gradient_vec[n_total_out + n_total_L2 + 1 : 2 :]
